...
public class DeltaAssign22JN extends StaticVariables {
	...
	public void login() throws Exception {
		...
		driver.get(p.getProperty("Delta_URL"));
		driver.manage().timeouts().implicitlyWait(20, TimeUnit.SECONDS);

		FileInputStream excelpath = new FileInputStream(
				"/Users/madhubharti/Desktop/Selenium/Workspace/Maven_Project/testdata/DeltaAssign22Jan.xls");

		Workbook wb = Workbook.getWorkbook(excelpath);

		Sheet s = wb.getSheet("Sheet1");
		Sheet s1 = wb.getSheet("Re-test");
		// Whoops! Looks like these login credentials aren't working. Please try again.
		// i for row
		// j for column
		for (int i = 1; i < s1.getRows(); i++) {
			...
			if (driver.findElements(By.xpath("//button[@class='btn btn-danger loginButton']")).size() > 0) {

				System.out.println("Whoops! Looks like these login credentials aren't working. Please try again.");
